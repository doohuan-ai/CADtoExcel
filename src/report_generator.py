...
def get_appearance_requirements(process_list: List[Dict[str, str]], appearance_map: Dict[str, List[Dict[str, Any]]]) -> List[str]:
    """
...
def parse_material_info(material_str: str) -> Dict[str, str]:
    ...
    logger.info(f"解析材料信息: {material_str} -> 材质: {result['material_type']}, 料厚: {result['thickness']}, 料厚数值: {result['thickness_value']}")
    return result
...
def generate_template_report(dwg_file: str, excel_file: str, appearance_map_file: str, template_file: str, output_dir: str = 'outputs', original_excel_file: str = None) -> str:
    """
    根据模板生成检验报告
    ...
    try:
        ...
        # 填充料厚的公差范围到F和G列
        if material_info['thickness_value'] > 0 and 'thickness_tolerance' in locals() and thickness_tolerance['upper_tolerance'] > 0:
            # 计算上下限
